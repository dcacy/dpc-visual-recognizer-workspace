describe('watsonworkspace-sdk', function () {
  ...
  it('getSpace', function (done) {
    ww.getSpace(spaceId, ['title',{name:'members',fields:[{name:'items',fields:['email']}]}])
    .then(space => expect(space.title).toBeDefined())
    .catch(error => expect(error).toBeUndefined())
    .finally(() => done())
  })

  // it('getSpaces', function (done) {
  //   //ww.getSpaces('items { id title members { items { email displayName }}}')
  //   ww.getSpaces(['id','title', {name:'members', fields: [{name:'items', fields:['email','displayName']}]}])
  //   .then(spaces => {
  //     console.log('spaces:', spaces);
  //     expect(spaces.length).toBeDefined()
  //   })
  //   .catch(error => expect(error).toBeUndefined())
  //   .finally(() => done())
  // })
