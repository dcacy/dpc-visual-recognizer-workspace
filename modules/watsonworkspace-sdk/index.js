...
module.exports = class SDK extends EventEmitter {
        ...
        oauth.run(
          ...
          (err, token) => {
            if (err) {
              ...
              setTimeout(new SDK(this.appId, this.Secret).authenticate, retry)
              return
            }

            if (this.token) {
              logger.info('Refreshed access token at', new Date())
            }
  ...
  /**
   * Get information about all spaces owned by the app.
   * @param {string[]} fields Fields returned in spaces list
   * @returns {Promise<Object>} Promise containing the array of space objects
   */
  getSpaces (fields) {
    const json = {
      query: graphql.getSpaces(fields)
    }

    // this.sendGraphql(json)
    // .then(result => {
    //   console.log('result is', result);
    //   return this.pick('items', this.pick('spaces', result))
    // });
    return this.pick('items', this.pick('spaces', this.sendGraphql(json)))
  }
